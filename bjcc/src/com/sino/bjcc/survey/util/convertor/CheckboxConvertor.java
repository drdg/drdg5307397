...
public class CheckboxConvertor extends HtmlDomConvertor {

	public String convertHtml(int index, Question question) {
		if (TYPE_CHECKBOX == question.getType()) {
			...
			for (int i = 0; i < optionArray.length; i++) {
				...
				buffer.append("<td><input type=checkbox class=qcheck name=").append(getDomName(index, false)).append(" value=").append(i).append(">");
			...
			if (question.getHasOther() == 1) {
				buffer.append("<tr>\n");
				int colspan = columns;
				buffer.append("<td colspan=").append(colspan).append("><input type=checkbox class=qcheck name=").append(getDomName(index, false)).append(" value=T").append(">");
